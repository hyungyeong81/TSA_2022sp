...
data.original = read.csv("practice2-2021sp.csv", head = FALSE) # change file name
head(data.original)

data.original = data.original$V2 # select a column
head(data.original)
...
data.added = data.original + 1000 # change the value in order to make all the values positive

data.log = log(data.original) # change the data if you need to transform data into positive values

data.sqrt = sqrt(data.original) # change the data if you need to transform data into positive values

library(MASS)
x = 1:length(data.original) # change the data if you need to transform data into positive values
fit = boxcox(data.original ~ 1) # change the data if you need to transform data into positive values
opt.lambda = fit$x[which.max(fit$y)]; opt.lambda
data.boxcox = data.original^opt.lambda # change the data if you need to transform data into positive values

layout(matrix(c(1, 1, 2, 3), 2, 2, byrow = TRUE))
plot(data.original, type = "l"); title("Time Plot") # change the input data into the transformed data
acf.plot = acf(data.original, lag = 50, plot = FALSE); plot(acf.plot, main = "SACF") # change the input data into the transformed data
pacf.plot = pacf(data.original, lag = 50, plot = FALSE); plot(pacf.plot, main = "SPACF") # change the input data into the transformed data
...
out.harmonic = lm(out.polynomial$residuals ~ 1 + costerm1 + sinterm1)
AIC(out.polynomial)
AIC(out.harmonic)
...
# auto.arima
library(forecast)
auto.arima(out.polynomial$residuals, d = 0)
...
fit.reg = Arima(data, order = c(1, 0, 8), xreg = X, include.mean = FALSE)
...
pmin = 0; qmin = 5 # change the minimum values
pmax = 8; qmax = 8 # change the maximum values
...
pmin = 0; qmin = 5 # change the minimum values
pmax = 5; qmax = 8 # change the maximum values
...
result2 = result.df # out-of-sample forecasting error model selection result


# GLS Regression with ARMA(6, 6)
arma.selected =  Arima(data, order = c(3, 0, 7), xreg = X, include.mean = FALSE)

tseries::adf.test(arma.selected$residuals, k = 100)
arma.selected

# Diagnostics: Coefficients - Are the coefficients significantly away from zero?
2*(1-pnorm(abs(arma.selected$coef[1:10]/(sqrt(diag(arma.selected$var.coef[1:10, 1:10])))))) < 0.05

arma.selected = Arima(data, order = c(3, 0, 7), xreg = X, include.mean = FALSE, fixed = c(NA, NA, 0, NA, NA,
                                                                                          NA, NA, NA, NA, NA,
                                                                                          NA, NA, NA))
...
####################
layout(matrix(c(1, 1, 2, 3), 2, 2, byrow = TRUE))
plot(diff(data), type = "l"); title("Time Plot")
acf.plot = acf(diff(data), lag = 50, plot = FALSE); plot(acf.plot, main = "SACF")
pacf.plot = pacf(diff(data), lag = 50, plot = FALSE); plot(pacf.plot, main = "SPACF")


# 차분을 너무 많이 하는 경우 과대차분으로 인해 분산이 커지는 문제 발생 - 차분은 1번만!
layout(matrix(c(1, 1, 2, 3), 2, 2, byrow = TRUE))
plot(diff(diff(diff(data))), type = "l"); title("Time Plot")
acf.plot = acf(diff(diff(diff(data))), lag = 50, plot = FALSE); plot(acf.plot, main = "SACF")
pacf.plot = pacf(diff(diff(diff(data))), lag = 50, plot = FALSE); plot(pacf.plot, main = "SPACF")
...
sarima.fit = arima(data, order = c(1, 1, 0), seasonal = list(order = c(0, 0, 1), period = 7))
...
for(i in 1:nrow(result.df)){
  ...
  tryCatch(expr = {
    sarima.fit = arima(data, order = c(p, 1, q), seasonal = list(order = c(P, 0, Q), period = 7)) # change the number of differencing and period
    m = p+q+P+Q+2
...
### RESULT : SARIMA(1, 1, 2)(0, 0, 1) with s = 7 is selected ###


###########################################
# Model Selection by Out-of-Sample Errors #
###########################################
...
for(j in 1:nrow(result.df)){
  ...
  tryCatch(expr = {
    for(i in 1:m){
      ...
      sarima.fit = arima(data[train.idx], order = c(p, 1, q), seasonal = list(order = c(P, 0, Q), period = 7), method = c("CSS-ML"))
...
sarima.selected = arima(data, order = c(1, 1, 2), seasonal = list(order = c(0, 0, 1), period = 7), method = c("CSS-ML"))
...
new.X = cbind(new.const, new.x1, new.x2)
colnames(new.X) <- c("col.const", "col.x1", "col.x2")

par(mfrow = c(1, 1))
plot(forecast(arma.selected, xreg = new.X, h = 30))
forecast(arma.selected, xreg = new.X[1:4, ], h = 4)


forecast(arma.selected, h = 4)
plot(forecast(arma.selected, h = 30))

# SARIMA
par(mfrow = c(1, 1))
plot(forecast(sarima.selected, h = 30))
forecast(sarima.selected, h = 4)
